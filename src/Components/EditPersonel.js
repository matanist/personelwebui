...
export default function EditPersonel(props) {
    ...
    useEffect(() => {
        ...
        var personid = props.match.params.id;
        const conf = { header: { "Content-Type": "application/json" } };
        axios.get(`${API_URL}Persons/${personid}`,conf)
            .then((data)=>{
                console.log(data.data);
                if(data.data==null)window.location.href="/";
                setSelectedPositionId(data.data.positionId);
    ...
    const btnYeniPersonelKaydetClicked = () => {
        ...
        const conf = { header: { "Content-Type": "application/json" } };
        axios.put(`${API_URL}Persons`, userInfo, conf)
            .then(data => {
                console.log(data.data);
        ...
        <div className="container mt-1">
            <form>
                <div className="form-group">
                    ...
                    <input type="text" className="form-control" id="txbFirstName" placeholder="Ad Giriniz" name="firstName" onChange={textChangeHandler} value={userInfo&&userInfo.firstName}/>
                </div>
                <div className="form-group">
                    <label htmlFor="txbLastName">Soyad</label>
                    <input type="text" className="form-control" id="txbLastName" placeholder="Soyad Giriniz" name="lastName" onChange={textChangeHandler} value={userInfo&&userInfo.lastName} />
                </div>
                <div className="form-group">
                    <label htmlFor="selectPosition">Position</label>
                    <select className="form-select" onChange={changePosition} name="positionId" value={selectedPositionId}>
                        {
